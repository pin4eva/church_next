...
import { CREATE_SERMON, UPDATE_SERMON } from "apollo/queries/sermonQuery";
...
const AddSermonModal = ({ onSuccess }) => {
  ...
  const [createSermon, { loading }] = useMutation(CREATE_SERMON);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const { data } = await createSermon({ variables: { input: sermonData } });
      setVideos([...videos, data.createSermon]);
      onSuccess(data.createSermon);
      alert("SUCCESS !");
    } catch (error) {
      console.log(error);
